...
fn do_test(data: &[u8]) {
    ...
    match tx_result {
        Err(_) => {}
        Ok(mut tx) => {
            let ser = unilayer::consensus::encode::serialize(&tx);
            assert_eq!(&ser[..], data);
            let len = ser.len();
            let calculated_weight = tx.weight().to_wu() as usize;
            for input in &mut tx.input {
                input.witness = unilayer::blockdata::witness::Witness::default();
            }
            let no_witness_len = unilayer::consensus::encode::serialize(&tx).len();
