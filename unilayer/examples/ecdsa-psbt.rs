...
//!
//! You can verify the workflow using `ulrd` and `ulr-cli`.
//!
//! ## Example Setup
//!
//! 1. Start UniLayer Core in Regtest mode, for example:
//!
//!    `ulrd -regtest -server -daemon -fallbackfee=0.0002 -rpcuser=admin -rpcpassword=pass -rpcallowip=127.0.0.1/0 -rpcbind=127.0.0.1 -blockfilterindex=1 -peerblockfilters=1`
//!
//! 2. Define a shell alias to `ulr-cli`, for example:
//!
//!    `alias ul=ulr-cli -rpcuser=admin -rpcpassword=pass -rpcport=18443`
//!
//! 3. Create (or load) a default wallet, for example:
//!
//!    `ul createwallet <wallet-name>`
//!
//! 4. Mine some blocks, for example:
//!
//!    `ul generatetoaddress 110 $(bt getnewaddress)`
//!
...
use unilayer::bip32::{ChildNumber, DerivationPath, Fingerprint, IntoDerivationPath, Xpriv, Xpub};
use unilayer::consensus::encode;
use unilayer::locktime::absolute;
use unilayer::psbt::{self, Input, Psbt, PsbtSighashType};
use unilayer::secp256k1::{Secp256k1, Signing, Verification};
use unilayer::{
...
const INPUT_UTXO_VALUE: &str = "50 ULR";
...
// These should be correct if the UTXO above should is for 50 ULR.
const OUTPUT_AMOUNT_ULR: &str = "1 ULR";
const CHANGE_AMOUNT_ULR: &str = "48.99999 ULR"; // 1000 sat transaction fee.
...
impl WatchOnly {
    ...
    ///
    /// A watch-only wallet would typically be online and connected to the UniLayer network. We
    /// 'import' into the wallet the `account_0_xpub` and `master_fingerprint`.
    ///
    /// The reason for importing the `input_xpub` is so one can use ulrd to grab a valid input
    ...
    fn create_psbt<C: Verification>(&self, secp: &Secp256k1<C>) -> Result<Psbt> {
        ...
        let change_amount = Amount::from_str(CHANGE_AMOUNT_ULR)?;
