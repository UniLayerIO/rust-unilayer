...
use unilayer::key::{Keypair, TapTweak, TweakedKeypair, UntweakedPublicKey};
use unilayer::locktime::absolute;
use unilayer::secp256k1::{rand, Message, Secp256k1, SecretKey, Signing, Verification};
use unilayer::sighash::{Prevouts, SighashCache, TapSighashType};
use unilayer::{
...
fn main() {
    ...
    // Sign the sighash using the secp256k1 library (exported by rust-unilayer).
    let tweaked: TweakedKeypair = keypair.tap_tweak(&secp, None);
    let msg = Message::from_digest(sighash.to_byte_array());
    let signature = secp.sign_schnorr(&msg, &tweaked.to_inner());

    // Update the witness stack.
    let signature = unilayer::taproot::Signature { signature, sighash_type };
