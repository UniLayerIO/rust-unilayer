...
// We use the alias `alias bt='ulr-cli -regtest'` for brevity.
...
//                  alias bt-benefactor='ulr-cli -regtest -rpcwallet=benefactor'
//                  alias bt-beneficiary='ulr-cli -regtest -rpcwallet=beneficiary'
//
...
const UTXO_1: P2trUtxo = P2trUtxo {
    ...
    amount_in_sats: Amount::from_int_ulr(50),
...
use unilayer::bip32::{ChildNumber, DerivationPath, Fingerprint, Xpriv, Xpub};
use unilayer::consensus::encode;
use unilayer::hashes::Hash;
use unilayer::key::{TapTweak, XOnlyPublicKey};
use unilayer::opcodes::all::{OP_CHECKSIG, OP_CLTV, OP_DROP};
use unilayer::psbt::{self, Input, Output, Psbt, PsbtSighashType};
use unilayer::secp256k1::Secp256k1;
use unilayer::sighash::{self, SighashCache, TapSighash, TapSighashType};
use unilayer::taproot::{self, LeafVersion, TapLeafHash, TaprootBuilder, TaprootSpendInfo};
use unilayer::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let change_address =
        Address::from_str("bcrt1pz449kexzydh2kaypatup5ultru3ej284t6eguhnkn6wkhswt0l7q3a7j76")?
            .require_network(Network::Regtest)?;
    let amount_to_send_in_sats = Amount::ONE_ULR;
...
fn generate_bip86_key_spend_tx(
    ...
    let tx1 = Transaction {
        ...
        input: vec![TxIn {
            ...
            sequence: unilayer::Sequence(0xFFFFFFFF), // Ignore nSequence.
    ...
    fn create_inheritance_funding_tx(
        ...
        let next_tx = Transaction {
            ...
            input: vec![TxIn {
                ...
                script_sig: ScriptBuf::new(),
                sequence: unilayer::Sequence(0xFFFFFFFD), // enable locktime and opt-in RBF
                witness: Witness::default(),
            }],
            output: vec![],
            validator_register: vec![],
