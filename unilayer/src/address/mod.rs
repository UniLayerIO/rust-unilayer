...
//! Bitcoin/UniLayer addresses.
//!
//! Support for ordinary base58 Bitcoin/UniLayer addresses and private keys.
//!
//! # Example: creating a new address from a randomly-generated key pair
//!
//! ```rust
//! # #[cfg(feature = "rand-std")] {
//! use unilayer::{Address, PublicKey, Network};
//! use unilayer::secp256k1::{rand, Secp256k1};
//!
...
//! let address = Address::p2pkh(&public_key, Network::UniLayer);
//! # }
//! ```
//!
...
//!
//! ```toml
//! unilayer = { version = "...", features = ["rand-std"] }
//! ```
...
/// use unilayer::address::{NetworkUnchecked, NetworkChecked};
///
/// // variant 1
/// let address: Address<NetworkUnchecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse().unwrap();
/// let address: Address<NetworkChecked> = address.require_network(Network::UniLayer).unwrap();
///
/// // variant 2
/// let address: Address = Address::from_str("32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf").unwrap()
///                .require_network(Network::UniLayer).unwrap();
///
/// // variant 3
/// let address: Address<NetworkChecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse::<Address<_>>()
///                .unwrap().require_network(Network::UniLayer).unwrap();
/// ```
///
...
/// ```
/// # use std::str::FromStr;
/// # use unilayer::address::{Address, NetworkChecked};
...
/// ```
/// # use std::str::FromStr;
/// # use unilayer::address::{Address, NetworkUnchecked};
/// let address: Address<NetworkUnchecked> = Address::from_str("132F25rTsvBdp9JzLLBHP5mvGY66i1xdiM")
///                .unwrap();
...
#[cfg(feature = "serde")]
crate::serde_utils::serde_string_deserialize_impl!(Address<NetworkUnchecked>, "a UniLayer address");
...
impl Address {
    ...
    /// Checks whether or not the address is following UniLayer standardness rules when
    ...
    /// Creates a URI string *unilayer:address* optimized to be encoded in QR codes.
    ///
    ...
    ///
    /// Note however that despite BIP21 explicitly stating that the `unilayer:` prefix should be
    ...
    /// # let address = ADDRESS.parse::<unilayer::Address<_>>().unwrap().assume_checked();
    ...
    /// ```
    pub fn to_qr_uri(&self) -> String { format!("unilayer:{:#}", self) }
...
impl Address<NetworkUnchecked> {
    ...
    /// ```rust
    /// use unilayer::{Address, Network};
    /// use unilayer::address::NetworkUnchecked;
    ///
    ...
    ///
    /// assert_eq!(address.is_valid_for_network(Network::UniLayer), false);
    ///
    /// let address: Address<NetworkUnchecked> = "32iVBEu4dxkUQk9dJbZUiBiQdmypcEyJRf".parse().unwrap();
    /// assert!(address.is_valid_for_network(Network::UniLayer));
...
mod tests {
    ...
    fn test_address_type() {
        ...
        for (address, expected_type) in &addresses {
            let addr = Address::from_str(address)
                .unwrap()
                .require_network(Network::UniLayer)
                .expect("mainnet");
    ...
    fn test_qr_string() {
        ...
        {
            let addr =
                Address::from_str(el).unwrap().require_network(Network::UniLayer).expect("mainnet");
            assert_eq!(addr.to_qr_uri(), format!("unilayer:{}", el));
        }
        ...
        .iter()
        {
            ...
            assert_eq!(addr.to_qr_uri(), format!("unilayer:{}", el.to_ascii_uppercase()));
        }
    }
    ...
    fn test_fail_address_from_script() {
        ...
        assert_eq!(Address::from_script(&bad_p2wpkh, Network::UniLayer), expected);
        assert_eq!(Address::from_script(&bad_p2wsh, Network::UniLayer), expected);
        assert_eq!(
            Address::from_script(&invalid_segwitv0_script, Network::UniLayer),
    ...
    fn test_matches_script_pubkey() {
        ...
        for addr in &addresses {
            let addr = Address::from_str(addr).unwrap().require_network(Network::UniLayer).unwrap();
            for another in &addresses {
                let another =
                    Address::from_str(another).unwrap().require_network(Network::UniLayer).unwrap();
