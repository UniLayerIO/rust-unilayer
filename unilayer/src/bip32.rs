...
/// Version bytes for extended public keys on the UniLayer network.
const VERSION_BYTES_MAINNET_PUBLIC: [u8; 4] = [0x04, 0x88, 0xB2, 0x1E];
/// Version bytes for extended private keys on the UniLayer network.
...
impl DerivationPath {
    ...
    ///
    /// ```
    /// use unilayer::bip32::{DerivationPath, ChildNumber};
    /// use std::str::FromStr;
    ///
    ...
    ///
    /// ```
    /// use unilayer::bip32::DerivationPath;
    /// use std::str::FromStr;
    ///
...
impl Xpriv {
    ...
    pub fn new_master(network: impl Into<NetworkKind>, seed: &[u8]) -> Result<Xpriv, Error> {
        let mut hmac_engine: HmacEngine<sha512::Hash> = HmacEngine::new(b"UniLayer seed");
