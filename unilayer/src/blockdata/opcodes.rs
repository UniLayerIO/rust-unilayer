...
//! Bitcoin/UniLayer script opcodes.
//!
//! Bitcoin's and UniLayer's script uses a stack-based assembly language. This module defines
...
///
///   Bitcoin/UniLayer Core's `IsPushOnly` considers `OP_RESERVED` to be a "push code", allowing this opcode
...
macro_rules! all_opcodes {
    ($($op:ident => $val:expr, $doc:expr);*) => {
        ...
        ///
        /// The `all` module is provided so one can use a wildcard import `use unilayer::opcodes::all::*` to
...
all_opcodes! {
    ...
    OP_ZEROCOINMINT => 0xc1, "Informational opcode, signalizes that the transaction will mint zULR.";
    OP_ZEROCOINSPEND => 0xc2, "Informational opcode, enables spending functionality for zULR.";
    OP_ZEROCOINPUBLICSPEND => 0xc3, "Informational opcode, enables public spending functionality for zULR.";
    ...
    OP_CLTV => 0xd9, "While UniLayer supports NOP2 as CLTV, only 0xd9 treats as a part of standard transaction instead of 0xb1";
    ...
    OP_SENDER => 0xe4, "EVM: Signal that selected output is signed for it to be verifyed. Actual UniLayer policy enforcing this opcode to be presented in case of EVM transaction. Do nothing with the stack.";
