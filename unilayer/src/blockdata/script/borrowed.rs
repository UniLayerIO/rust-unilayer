...
/// UniLayer script slice.
///
/// *[See also the `unilayer::blockdata::script` module](crate::blockdata::script).*
///
...
impl Script {
    ...
    fn minimal_non_dust_inner(&self, dust_relay_fee: u128) -> crate::Amount {
        // This must never be lower than UniLayer Core's GetDustThreshold() (as of v1) as it may
    ...
    ///
    /// In UniLayer Core, there are two ways to count sigops, "accurate" and "legacy".
    ...
    ///
    /// UniLayer Core uses accurate counting for sigops contained within redeemScripts (P2SH)
    ...
    fn count_sigops_internal(&self, accurate: bool) -> usize {
        ...
        for inst in self.instructions() {
            match inst {
                ...
                // In UniLayer Core it does `if (!GetOp(pc, opcode)) break;`
                Err(_) => break,
            }
        }
    ...
    pub(crate) fn last_pushdata(&self) -> Option<&PushBytes> {
        match self.instructions().last() {
            ...
            // OP_16 (0x60) and lower are considered "pushes" by UniLayer Core (excl. OP_RESERVED).
