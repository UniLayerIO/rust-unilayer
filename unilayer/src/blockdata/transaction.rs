...
hashes::hash_newtype! {
    /// A unilayer transaction hash/transaction ID.
    ///
    ...
    /// A unilayer witness transaction ID.
...
impl OutPoint {
    ...
    ///
    /// let block = genesis_block(Network::UniLayer);
...
/// UniLayer transaction input sequence number.
///
...
impl Transaction {
    ...
    // Actual UniLayer code reference would be provided later.
    /// Maximum transaction weight for UniLayer Core 1.0.1.
    ...
    /// is different to what is implemented in UniLayer Core. The computation should be the same for
...
mod tests {
    ...
    fn is_coinbase() {
        ...
        let genesis = constants::genesis_block(Network::UniLayer);
    ...
    fn nonsegwit_transaction() {
        ...
        // In particular this one is easy to get backward -- in unilayer hashes are encoded
    ...
    fn effective_value_happy_path() {
        let value = Amount::from_str("1 cULR").unwrap();
