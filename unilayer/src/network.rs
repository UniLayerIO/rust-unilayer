...
//! UniLayer network we are operating on e.g., signet, regtest. The terms
...
//! use unilayer::consensus::encode::serialize;
//!
...
impl NetworkKind {
    /// Returns true if this is real mainnet UniLayer.
...
pub enum Network {
    /// Mainnet UniLayer.
    UniLayer,
    /// UniLayer's testnet network.
    Testnet,
    /// UniLayer's signet network. <- NOTE: currently is unsupported
    Signet,
    /// UniLayer's regtest network.
    Regtest,
}
...
impl Network {
    ...
    ///
    /// ```rust
    /// use unilayer::p2p::Magic;
    /// use unilayer::Network;
    ///
    /// assert_eq!(Ok(Network::UniLayer), Network::try_from(Magic::from_bytes([0x55, 0x6e, 0x69, 0x4d])));
    ...
    /// Converts a `Network` to its equivalent `ulrd -chain` argument name.
    ///
    /// ```bash
    /// $ ulr-1.0.1/bin/ulrd --help | grep -C 3 '\-chain=<chain>'
    ...
    /// Converts a `ulrd -chain` argument name to its equivalent `Network`.
    ///
    /// ```bash
    /// $ ulr-1.0.1/bin/ulrd --help | grep -C 3 '\-chain=<chain>'
    /// Chain selection options:
    ...
    /// ```rust
    /// use unilayer::Network;
    /// use unilayer::blockdata::constants::ChainHash;
    ///
    /// let network = Network::UniLayer;
    ...
    /// use unilayer::blockdata::constants::ChainHash;
    ///
    /// assert_eq!(Ok(Network::UniLayer), Network::try_from(ChainHash::UNILAYER));
    /// ```
...
#[cfg(feature = "serde")]
pub mod as_core_arg {
    //! Module for serialization/deserialization of network variants into/from UniLayer Core values
    #![allow(missing_docs)]
            ...
            fn visit_str<E: serde::de::Error>(self, s: &str) -> Result<Self::Value, E> {
                Network::from_core_arg(s).map_err(|_| {
                    E::invalid_value(
                        ...
                        &"unilayer network encoded as a string (either main, test, signet or regtest)",
                    )
                })
            }
...
mod tests {
    ...
    #[test]
    fn serialize_test() {
        assert_eq!(serialize(&Network::UniLayer.magic()), &[0x55, 0x6e, 0x69, 0x4d]);
        ...
        assert_eq!(deserialize(&[0x55, 0x6e, 0x69, 0x4d]).ok(), Some(Network::UniLayer.magic()));
    ...
    #[test]
    fn string_test() {
        assert_eq!(Network::UniLayer.to_string(), "main");
        assert_eq!(Network::Testnet.to_string(), "testnet");
        assert_eq!(Network::Regtest.to_string(), "regtest");
        assert_eq!(Network::Signet.to_string(), "signet");

        assert_eq!("mainnet".parse::<Network>().unwrap(), Network::UniLayer);
