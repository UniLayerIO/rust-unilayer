...
//! Bitcoin/UniLayer network messages.
//!
//! This module defines the `NetworkMessage` and `RawNetworkMessage` types that
//! are used for (de)serializing UniLayer objects for transmission on the network.
//!
...
mod test {
    ...
    fn serialize_verack_test() {
        assert_eq!(serialize(&RawNetworkMessage::new(Magic::UNILAYER, NetworkMessage::Verack)),
    ...
    fn serialize_ping_test() {
        assert_eq!(serialize(&RawNetworkMessage::new(Magic::UNILAYER, NetworkMessage::Ping(100))),
    ...
    fn serialize_mempool_test() {
        assert_eq!(serialize(&RawNetworkMessage::new(Magic::UNILAYER, NetworkMessage::MemPool)),
    ...
    fn serialize_getaddr_test() {
        assert_eq!(serialize(&RawNetworkMessage::new(Magic::UNILAYER, NetworkMessage::GetAddr)),
    ...
    fn deserialize_version_test() {
        ...
        assert!(msg.is_ok());
        let msg = msg.unwrap();
        assert_eq!(msg.magic, Magic::UNILAYER);
