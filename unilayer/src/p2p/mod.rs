...
//! Bitcoin/UniLayer p2p network types.
//!
//! This module defines support for (de)serialization and network transport
//! of UniLayer data and UniLayer p2p network messages.
...
///
/// UniLayer operates on 70918 version.
...
impl ServiceFlags {
    ...
    /// set by all UniLayer Core non pruned nodes, and is unset by SPV clients or other light
    /// clients.
...
impl Magic {
    /// UniLayer mainnet network magic bytes.
    pub const UNILAYER: Self = Self([0x55, 0x6e, 0x69, 0x4d]);
    /// UniLayer testnet network magic bytes.
    pub const TESTNET: Self = Self([0x55, 0x6e, 0x69, 0x54]);
    /// UniLayer signet network magic bytes.
    pub const SIGNET: Self = Self([0x55, 0x6e, 0x69, 0x71]);
    /// UniLayer regtest network magic bytes.
...
impl From<Network> for Magic {
    fn from(network: Network) -> Magic {
        match network {
            ...
            Network::UniLayer => Magic::UNILAYER,
...
impl TryFrom<Magic> for Network {
    ...
    fn try_from(magic: Magic) -> Result<Self, Self::Error> {
        match magic {
            ...
            Magic::UNILAYER => Ok(Network::UniLayer),
