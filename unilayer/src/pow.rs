...
impl Work {
    ...
    /// used mainly for informative and displaying purposes, similarly to UniLayer Core's
...
impl Target { // TODO: ensure correct values
    ...
    ///
    /// ref: <https://en.bitcoin.it/wiki/Target>
    // In Ulrd this is ~(u256)0 >> 32 stored as a floating-point type so it gets truncated, hence
    ...
    /// defined for the UniLayer network and `current` is the current [target] for this block. As
    ...
    pub fn difficulty(&self, network: Network) -> u128 {
        let max = match network {
            Network::UniLayer => Target::MAX_ATTAINABLE_MAINNET,
