...
//! Partially Signed Bitcoin Transactions for UniLayer Network.
//!
...
impl Psbt {
    ...
    fn internal_extract_tx_with_fee_rate_limit(
        ...
        let fee_rate = FeeRate::from_sat_per_kwu(
            fee.to_sat().saturating_mul(1000) / (tx.weight().to_wu() as u128),
        );
...
/// The various output types supported by the UniLayer network.
...
/// Signing algorithms supported by the UniLayer network.
#[derive(Clone, Copy, Debug, PartialEq, Eq, PartialOrd, Ord, Hash)]
