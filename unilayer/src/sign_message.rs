...
/// The prefix for signed messages using UniLayer's message signing protocol.
pub const BITCOIN_SIGNED_MSG_PREFIX: &[u8] = b"\x18UniLayer Signed Message:\n";
...
mod message_signing {
    ...
    /// An error used for dealing with UniLayer Signed Messages.
    ...
    /// A signature on a UniLayer Signed Message.
    ///
...
/// Hash message for signature using UniLayer's message signing format.
...
mod tests {
    ...
    fn test_message_signature() {
        ...
        let message = "rust-unilayer MessageSignature test";
        ...
        let p2wpkh = Address::p2wpkh(&pubkey, Network::UniLayer);
        assert_eq!(
        ...
        let p2pkh = Address::p2pkh(pubkey, Network::UniLayer);
        assert_eq!(signature2.is_signed_by_address(&secp, &p2pkh, msg_hash), Ok(true));
    ...
    fn test_incorrect_message_signature() {
        ...
        // Signature of msg = "rust-unilayer MessageSignature test"
