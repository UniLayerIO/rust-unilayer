...
use unilayer::bip32::{Fingerprint, IntoDerivationPath, KeySource, Xpriv, Xpub};
use unilayer::blockdata::opcodes::OP_0;
use unilayer::blockdata::{script, transaction};
use unilayer::consensus::encode::{deserialize, serialize_hex};
use unilayer::hex::FromHex;
use unilayer::psbt::{Psbt, PsbtSighashType};
use unilayer::script::PushBytes;
...
fn create_transaction() -> Transaction {
    ...
    Transaction {
        ...
        output: vec![
            TxOut {
                value: Amount::from_str_in(output_0.amount, Denomination::ULR)
                    .expect("failed to parse amount"),
                script_pubkey: ScriptBuf::from_hex(output_0.script_pubkey)
                    .expect("failed to parse script"),
            },
            TxOut {
                value: Amount::from_str_in(output_1.amount, Denomination::ULR)
                    .expect("failed to parse amount"),
...
#[track_caller]
fn signer_one_sign(psbt: Psbt, key_map: BTreeMap<unilayer::PublicKey, PrivateKey>) -> Psbt {
...
#[track_caller]
fn signer_two_sign(psbt: Psbt, key_map: BTreeMap<unilayer::PublicKey, PrivateKey>) -> Psbt {
...
fn sign(mut psbt: Psbt, keys: BTreeMap<unilayer::PublicKey, PrivateKey>) -> Psbt {
    let secp = Secp256k1::new();
